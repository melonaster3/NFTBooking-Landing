...
class MultiwrapImpl extends dist_thirdwebDevSdk.StandardErc721 {
  /**
   * Configure royalties
  ...
  constructor(network, address, storage) {
    ...
    this.abi = abi;
    this.metadata = new dist_thirdwebDevSdk.ContractMetadata(this.contractWrapper, dist_thirdwebDevSdk.MultiwrapContractSchema, this.storage);
    this.roles = new dist_thirdwebDevSdk.ContractRoles(this.contractWrapper, MultiwrapImpl.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await dist_thirdwebDevSdk.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
  ...
  async wrap(contents, wrappedTokenMetadata, recipientAddress) {
    const uri = await dist_thirdwebDevSdk.uploadOrExtractURI(wrappedTokenMetadata, this.storage);
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await dist_thirdwebDevSdk.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
        const hasAllowance = await dist_thirdwebDevSdk.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, normalizedQuantity);
...
dist_thirdwebDevSdk._defineProperty(MultiwrapImpl, "contractRoles", ["transfer", "minter", "unwrap", "asset"]);

exports.MultiwrapImpl = MultiwrapImpl;
