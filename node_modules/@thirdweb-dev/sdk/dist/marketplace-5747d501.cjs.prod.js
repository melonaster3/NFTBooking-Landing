...
class MarketplaceImpl {
  ...
  constructor(network, address, storage) {
    ...
    dist_thirdwebDevSdk._defineProperty(this, "contractWrapper", void 0);

    dist_thirdwebDevSdk._defineProperty(this, "storage", void 0);
    ...
    dist_thirdwebDevSdk._defineProperty(this, "direct", void 0);

    dist_thirdwebDevSdk._defineProperty(this, "auction", void 0);

    dist_thirdwebDevSdk._defineProperty(this, "getAll", this.getAllListings);

    this.abi = abi;
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.metadata = new dist_thirdwebDevSdk.ContractMetadata(this.contractWrapper, dist_thirdwebDevSdk.MarketplaceContractSchema, this.storage);
    this.roles = new dist_thirdwebDevSdk.ContractRoles(this.contractWrapper, MarketplaceImpl.contractRoles);
    ...
    this.direct = new dist_thirdwebDevSdk.MarketplaceDirect(this.contractWrapper, this.storage);
    this.auction = new dist_thirdwebDevSdk.MarketplaceAuction(this.contractWrapper, this.storage);
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new dist_thirdwebDevSdk.ListingNotFoundError(this.getAddress(), listingId.toString());
    }

    switch (listing.listingType) {
      case dist_thirdwebDevSdk.ListingType.Auction:
        {
          return await this.auction.mapListing(listing);
        }

      case dist_thirdwebDevSdk.ListingType.Direct:
        {
  ...
  async getActiveListings(filter) {
    ...
    return filtered.filter(l => {
      return l.type === dist_thirdwebDevSdk.ListingType.Auction && ethers.BigNumber.from(l.endTimeInEpochSeconds).gt(now) && ethers.BigNumber.from(l.startTimeInEpochSeconds).lte(now) || l.type === dist_thirdwebDevSdk.ListingType.Direct && l.quantity > 0;
    });
  }
  /**
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(dist_thirdwebDevSdk.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async allowListingFromSpecificAssetOnly(contractAddress) {
    ...
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [dist_thirdwebDevSdk.getRoleHash("asset"), ethers.constants.AddressZero]));
    }

    encoded.push(this.encoder.encode("grantRole", [dist_thirdwebDevSdk.getRoleHash("asset"), contractAddress]));
  ...
  async allowListingFromAnyAsset() {
    ...
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [dist_thirdwebDevSdk.getRoleHash("asset"), addr]));
    }

    encoded.push(this.encoder.encode("grantRole", [dist_thirdwebDevSdk.getRoleHash("asset"), ethers.constants.AddressZero]));
    await this.contractWrapper.multiCall(encoded);
  }
  /** ******************************
   * PRIVATE FUNCTIONS
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof dist_thirdwebDevSdk.ListingNotFoundError) {
          return undefined;
        } else {
      ...
      if (listing.type === dist_thirdwebDevSdk.ListingType.Auction) {
        return listing;
      }
  ...
  applyFilter(listings, filter) {
    ...
    const count = ethers.BigNumber.from((filter === null || filter === void 0 ? void 0 : filter.count) || dist_thirdwebDevSdk.DEFAULT_QUERY_ALL_COUNT).toNumber();
...
dist_thirdwebDevSdk._defineProperty(MarketplaceImpl, "contractRoles", ["admin", "lister", "asset"]);

exports.MarketplaceImpl = MarketplaceImpl;
