...
function getProviderForNetwork(network) {
  ...
  switch (network) {
    ...
    case "optimism-goerli":
      // TODO test this RPC
      return "https://opt-goerli.g.alchemy.com/v2/".concat(DEFAULT_API_KEY);
    ...
    case "arbitrum-goerli":
      // TODO test this RPC
      return "https://arb-goerli.g.alchemy.com/v2/".concat(DEFAULT_API_KEY);
...
class ExtensionNotImplementedError extends Error {
  constructor(feature) {
    super("This functionality is not available because the contract does not implement the '".concat(feature.docLinks.contracts, "' Extension. Learn how to unlock this functionality at https://portal.thirdweb.com/extensions "));
  }
...
async function extractIPFSHashFromBytecode(bytecode) {
  ...
  const bytecodeBuffer = Buffer.from(numericBytecode.slice(numericBytecode.length - 2 - cborLength, -2)); // load these lazily to avoid loading them when they are not needed
...
class ContractWrapper extends RPCConnectionHandler {
  ...
  async call(functionName) {
    ...
    const ethersFnName = "".concat(functionName, "(").concat(fn.inputs.map(i => i.type).join(), ")"); // check if the function exists on the contract, otherwise use the name passed in
...
class Erc721 {
  ...
  async getOwned(walletAddress) {
    ...
    if ((_this$query = this.query) !== null && _this$query !== void 0 && _this$query.owned) {
      ...
      return (allNFTs || []).filter(_ref => {
        let {
          owner
        } = _ref;
        return (walletAddress === null || walletAddress === void 0 ? void 0 : walletAddress.toLowerCase()) === (owner === null || owner === void 0 ? void 0 : owner.toLowerCase());
      });
    }
  }
  /**
...
const EditionDrop = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([EditionDrop.getAbi(), import('./edition-drop-25b3bb5b.esm.js')]);
    return new contract.EditionDropImpl(network, address, storage, options, abi);
  },
...
const Edition = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Edition.getAbi(), import('./edition-5f78023f.esm.js')]);
...
const Marketplace = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Marketplace.getAbi(), import('./marketplace-15b2ad3c.esm.js')]);
    return new contract.MarketplaceImpl(network, address, storage, options, abi);
  },
...
const Multiwrap = {
  ...
  roles: ["transfer", "minter", "unwrap", "asset"],
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Multiwrap.getAbi(), import('./multiwrap-a6e3ba6a.esm.js')]);
...
const NFTCollection = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([NFTCollection.getAbi(), import('./nft-collection-c1f00425.esm.js')]);
    return new contract.NFTCollectionImpl(network, address, storage, options, abi);
  },
...
const NFTDrop = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([NFTDrop.getAbi(), import('./nft-drop-7c7c9c11.esm.js')]);
...
const Pack = {
  ...
  roles: ["admin", "minter", "pauser", "transfer"],
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Pack.getAbi(), import('./pack-ef8b27e6.esm.js')]);
...
const SignatureDrop = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([SignatureDrop.getAbi(), import('./signature-drop-b6e35c67.esm.js')]);
    return new contract.SignatureDropImpl(network, address, storage, options, abi);
  },
...
const Split = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Split.getAbi(), import('./split-1aa7556b.esm.js')]);
...
const TokenDrop = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([TokenDrop.getAbi(), import('./token-drop-88ae15fe.esm.js')]);
...
const Token = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Token.getAbi(), import('./token-fbe67654.esm.js')]);
...
const Vote = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([Vote.getAbi(), import('./vote-89db6398.esm.js')]);
...
class SmartContract {
  ...
  constructor(network, address, abi, storage) {
    ...
    this.metadata = new ContractMetadata(this.contractWrapper, CustomContractSchema, this.storage); // feature detection
...
class ThirdwebSDK extends RPCConnectionHandler {
  ...
  async getSignatureDrop(contractAddress) {
    return await this.getBuiltInContract(contractAddress, SignatureDrop.contractType);
  }
  /**
...
class ContractFactory extends ContractWrapper {
  ...
  async getDeployArguments(contractType, metadata, contractURI) {
    let trustedForwarders = contractType === Pack.contractType ? [] : await this.getDefaultTrustedForwarders(); // override default forwarders if custom ones are passed in
...
export { TokenERC20History as $, Erc721WithQuantitySignatureMintable as A, DropErc721ContractSchema as B, ContractWrapper as C, DropErc1155ContractSchema as D, Erc1155 as E, DropClaimConditions as F, GasCostEstimator as G, DelayedReveal as H, FEATURE_NFT_REVEALABLE as I, uploadOrExtractURIs as J, prepareClaim as K, ListingNotFoundError as L, MarketplaceContractSchema as M, AddressSchema as N, AmountSchema as O, BigNumberishSchema as P, NFTInputOrUriSchema as Q, RawDateSchema as R, StandardErc1155 as S, TransactionTask as T, PackContractSchema as U, SplitsContractSchema as V, fetchCurrencyValue as W, StandardErc20 as X, DropErc20ContractSchema as Y, TokenErc20ContractSchema as Z, _defineProperty as _, ContractMetadata as a, CompilerMetadataFetchedSchema as a$, Erc20SignatureMintable as a0, VoteContractSchema as a1, VoteType as a2, CommonContractSchema as a3, QuantitySchema as a4, ClaimConditionInputSchema as a5, ClaimConditionInputArray as a6, PartialClaimConditionInputSchema as a7, ClaimConditionOutputSchema as a8, OptionalPropertiesInput as a9, MintRequest721 as aA, MintRequest1155 as aB, MintRequest721withQuantity as aC, MerkleSchema as aD, SnapshotAddressInput as aE, SnapshotInputSchema as aF, SnapshotSchema as aG, SnapshotInfoSchema as aH, BYOCContractMetadataSchema as aI, CustomContractInput as aJ, CustomContractOutput as aK, CustomContractDeploy as aL, CustomContractSchema as aM, AbiTypeSchema as aN, AbiObjectSchema as aO, AbiSchema as aP, PreDeployMetadata as aQ, ChainIdToAddressSchema as aR, FactoryDeploymentSchema as aS, ExtraPublishMetadataSchemaInput as aT, ExtraPublishMetadataSchemaOutput as aU, FullPublishMetadataSchemaInput as aV, FullPublishMetadataSchemaOutput as aW, ProfileSchemaInput as aX, ProfileSchemaOutput as aY, PublishedContractSchema as aZ, ContractInfoSchema as a_, ChainId as aa, SUPPORTED_CHAIN_IDS as ab, TokenMintInputSchema as ac, EditionMetadataOutputSchema as ad, EditionMetadataWithOwnerOutputSchema as ae, EditionMetadataInputSchema as af, EditionMetadataInputOrUriSchema as ag, CommonContractOutputSchema as ah, CommonRoyaltySchema as ai, CommonPrimarySaleSchema as aj, CommonPlatformFeeSchema as ak, CommonTrustedForwarderSchema as al, CommonSymbolSchema as am, CurrencySchema as an, CurrencyValueSchema as ao, BaseSignaturePayloadInput as ap, Signature20PayloadInput as aq, Signature20PayloadOutput as ar, Signature721PayloadInput as as, Signature721PayloadOutput as at, Signature1155PayloadInput as au, Signature1155PayloadInputWithTokenId as av, Signature1155PayloadOutput as aw, Signature721WithQuantityInput as ax, Signature721WithQuantityOutput as ay, MintRequest20 as az, ContractRoles as b, fetchContractMetadataFromAddress as b$, PreDeployMetadataFetchedSchema as b0, LoginOptionsSchema as b1, LoginPayloadDataSchema as b2, LoginPayloadSchema as b3, VerifyOptionsSchema as b4, AuthenticationOptionsSchema as b5, AuthenticationPayloadDataSchema as b6, AuthenticationPayloadSchema as b7, WalletAuthenticator as b8, Erc20 as b9, FileNameMissingError as bA, DuplicateFileNameError as bB, NotEnoughTokensError as bC, MissingOwnerRoleError as bD, QuantityAboveLimitError as bE, FetchError as bF, DuplicateLeafsError as bG, AuctionAlreadyStartedError as bH, FunctionDeprecatedError as bI, WrongListingTypeError as bJ, RestrictedTransferError as bK, AdminRoleMissingError as bL, AuctionHasNotEndedError as bM, ExtensionNotImplementedError as bN, TransactionError as bO, convertToTWError as bP, includesErrorMessage as bQ, createSnapshot as bR, hashLeafNode as bS, ALL_ROLES as bT, extractConstructorParams as bU, extractFunctions as bV, extractConstructorParamsFromAbi as bW, extractFunctionParamsFromAbi as bX, extractFunctionsFromAbi as bY, extractEventsFromAbi as bZ, resolveContractUriFromAddress as b_, Erc20Mintable as ba, Erc20BatchMintable as bb, Erc20Burnable as bc, Erc721LazyMintable as bd, Erc721ClaimableWithConditions as be, Erc721Supply as bf, Erc721Enumerable as bg, Erc721Mintable as bh, Erc721BatchMintable as bi, Erc721Burnable as bj, Erc1155Enumerable as bk, Erc1155Mintable as bl, Erc1155BatchMintable as bm, Erc1155Burnable as bn, Erc1155LazyMintable as bo, ContractDeployer as bp, ContractPublishedMetadata as bq, UserWallet as br, ThirdwebSDK as bs, ProposalState as bt, ClaimEligibility as bu, NotFoundError as bv, InvalidAddressError as bw, MissingRoleError as bx, AssetNotFoundError as by, UploadError as bz, ContractRoyalty as c, fetchContractMetadata as c0, fetchSourceFilesFromMetadata as c1, fetchRawPredeployMetadata as c2, fetchPreDeployMetadata as c3, fetchExtendedReleaseMetadata as c4, detectFeatures as c5, isFeatureEnabled as c6, assertEnabled as c7, detectContractFeature as c8, hasFunction as c9, SignatureDrop as cA, Split as cB, TokenDrop as cC, Token as cD, Vote as cE, PREBUILT_CONTRACTS_MAP as cF, CONTRACTS_MAP as cG, getContractTypeForRemoteName as cH, toSemver as ca, isIncrementalVersion as cb, isDowngradeVersion as cc, OZ_DEFENDER_FORWARDER_ADDRESS as cd, CONTRACT_ADDRESSES as ce, getContractAddressByChainId as cf, getContractPublisherAddress as cg, InterfaceId_IERC721 as ch, InterfaceId_IERC1155 as ci, NATIVE_TOKEN_ADDRESS as cj, NATIVE_TOKENS as ck, getNativeTokenByChainId as cl, EventType as cm, DEFAULT_IPFS_GATEWAY as cn, PUBLIC_GATEWAYS as co, TW_IPFS_SERVER_URL as cp, PINATA_IPFS_URL as cq, getProviderForNetwork as cr, getReadOnlyProvider as cs, EditionDrop as ct, Edition as cu, Marketplace as cv, Multiwrap as cw, NFTCollection as cx, NFTDrop as cy, Pack as cz, ContractPrimarySale as d, DropErc1155ClaimConditions as e, ContractEvents as f, DropErc1155History as g, ContractEncoder as h, ContractPlatformFee as i, ContractInterceptor as j, getRoleHash as k, TokenErc1155ContractSchema as l, Erc1155SignatureMintable as m, MarketplaceDirect as n, MarketplaceAuction as o, ListingType as p, DEFAULT_QUERY_ALL_COUNT as q, StandardErc721 as r, MultiwrapContractSchema as s, fetchCurrencyMetadata as t, uploadOrExtractURI as u, normalizePriceValue as v, hasERC20Allowance as w, isTokenApprovedForTransfer as x, TokenErc721ContractSchema as y, Erc721 as z };
