...
const CommonWrappableSchema = zod.z.object({
  contractAddress: dist_thirdwebDevSdk.AddressSchema
});
/**
 * @internal
 */
...
const ERC20WrappableSchema = CommonWrappableSchema.extend({
  quantity: dist_thirdwebDevSdk.AmountSchema
});
/**
 * @internal
 */
...
const ERC721WrappableSchema = CommonWrappableSchema.extend({
  tokenId: dist_thirdwebDevSdk.BigNumberishSchema
});
/**
 * @internal
 */
...
const ERC1155WrappableSchema = CommonWrappableSchema.extend({
  tokenId: dist_thirdwebDevSdk.BigNumberishSchema,
  quantity: dist_thirdwebDevSdk.BigNumberishSchema
});
...
const ERC20RewardSchema = ERC20WrappableSchema.omit({
  ...
  quantityPerReward: dist_thirdwebDevSdk.AmountSchema
});
/**
 * @internal
 */
...
const ERC1155RewardSchema = ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: dist_thirdwebDevSdk.BigNumberishSchema
});
/**
 * @internal
 */
...
const ERC20RewardContentsSchema = ERC20RewardSchema.extend({
  totalRewards: dist_thirdwebDevSdk.BigNumberishSchema.default("1")
});
/**
 * @internal
 */
...
const PackMetadataInputSchema = PackRewardsOutputSchema.extend({
  packMetadata: dist_thirdwebDevSdk.NFTInputOrUriSchema,
  rewardsPerPack: dist_thirdwebDevSdk.BigNumberishSchema.default("1"),
  openStartTime: dist_thirdwebDevSdk.RawDateSchema.default(new Date())
});
/**
 * @public
 */
...
class PackImpl extends dist_thirdwebDevSdk.StandardErc1155 {
  /**
   * Configure royalties
  ...
  constructor(network, address, storage) {
    ...
    dist_thirdwebDevSdk._defineProperty(this, "erc1155", void 0);

    this.abi = abi;
    this.erc1155 = new dist_thirdwebDevSdk.Erc1155(this.contractWrapper, this.storage);
    this.metadata = new dist_thirdwebDevSdk.ContractMetadata(this.contractWrapper, dist_thirdwebDevSdk.PackContractSchema, this.storage);
    this.roles = new dist_thirdwebDevSdk.ContractRoles(this.contractWrapper, PackImpl.contractRoles);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(dist_thirdwebDevSdk.getRoleHash("transfer"), ethers.ethers.constants.AddressZero);
  ...
  async getPackContents(packId) {
    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await dist_thirdwebDevSdk.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
  ...
  async createTo(to, metadataWithRewards) {
    const uri = await dist_thirdwebDevSdk.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await dist_thirdwebDevSdk.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress); // Multiply the quantity of one reward by the number of rewards

      const totalQuantity = normalizedQuantity.mul(erc20.totalRewards);
      const hasAllowance = await dist_thirdwebDevSdk.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, totalQuantity);
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await dist_thirdwebDevSdk.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await dist_thirdwebDevSdk.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
...
dist_thirdwebDevSdk._defineProperty(PackImpl, "contractRoles", ["admin", "minter", "pauser", "transfer"]);
